...
export function syncProperty<T>(el: HTMLElement | undefined, prop: string, value: T): void {
  if (!browser || !el) return;
  (el as unknown as Record<string, unknown>)[prop] = value;
}

export function syncManagedProperty<T>(el: HTMLElement | undefined, prop: string, value: T): void {
  if (!browser || !el) return;
