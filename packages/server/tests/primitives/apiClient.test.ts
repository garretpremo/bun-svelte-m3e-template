...
describe("createApiClient", () => {
  ...
  test("dispatches GET /api/notes with query params", async () => {
    const captured: { url: string | null; init: RequestInit | undefined } = { url: null, init: undefined };
  ...
  test("dispatches POST /api/notes with body and parses response", async () => {
    ...
    globalThis.fetch = mock(async () =>
      new Response(JSON.stringify(fakeNote), { status: 201, headers: { "content-type": "application/json" } })
    ) as unknown as typeof fetch;
